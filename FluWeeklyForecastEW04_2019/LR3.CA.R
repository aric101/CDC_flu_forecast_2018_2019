...
plot.ILI <- function(){
plot((0:130)/10, sim.test.ili[,1], type="l", main="TEST: CA flu peak 2015/2016, 2016/2018", xlab="%ILI", ylab="density", ylim=c(0,0.1))
...
plot((0:130)/10, sim.forecast.ili[,1], type="l", main="FORECAST: CA flu peak 2018/2019", xlab="%ILI", ylab="density", ylim=c(0,0.1))
...
png("./plots/CA pk prev forecast.png",width=6,height=8,units="in",res=150)
...
sapply(1:length(sim.forecast.ili.LIST), function(k)apply(sim.forecast.ili.LIST[[k]], 2, function(x)sum(x*(0:130))))
...
write.csv(national4wks, "./output/ca.pk.prev.csv", row.names=F)
