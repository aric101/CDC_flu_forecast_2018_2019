...
plot.ILI <- function(){
plot(sim.test.ili[,1], type="l", main="TEST: CA flu onset wk 2014/2015, 2015/2016", xlab="%ILI", ylab="density", ylim=c(0,0.1))
...
plot( sim.forecast.ili[,1], type="l", main="FORECAST: CA flu onset wk 2016/2018", xlab="%ILI", ylab="density", ylim=c(0,0.1))
...
png("./plots/CA onset wk forecast.png",width=6,height=8,units="in",res=150)
...
sapply(1:length(sim.forecast.ili.LIST), function(k)apply(sim.forecast.ili.LIST[[k]], 2, function(x)sum(x*(1:34))))
...
write.csv(national4wks, "./output/ca.onset.wk.csv", row.names=F)
